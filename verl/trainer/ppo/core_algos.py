...
def compute_grpo_outcome_advantage(
    ...
    token_level_log_probs: Optional[torch.Tensor] = None,
    ...
    Args:
        ...
        token_level_log_probs: `(Optional[torch.Tensor])`
            shape is (bs, response_length). Log probabilities from rollout policy.
            When provided, uses inverse probability weighted baseline calculation.

    Note:
        If norm_adv_by_std_in_grpo is True, the advantage is scaled by the std, as in the original GRPO.
        If False, the advantage is not scaled, as in Dr.GRPO (https://arxiv.org/abs/2503.20783).
        
        When token_level_log_probs is provided, the baseline is computed as inverse probability
        weighted mean: baseline = sum(w_i * R_i) / sum(w_i) where w_i = 1/π_i.
        This gives more weight to low-probability (surprising) samples.
    ...
    id2weight = defaultdict(list)  # For IPW baseline
    id2mean = {}
    id2std = {}
    ...
    with torch.no_grad():
        ...
        use_ipw = False
        # Check if IPW is enabled in config
        if config is not None and getattr(config, "use_ipw", False):
            if token_level_log_probs is not None:
                # Compute sequence-level log probability: sum of token log probs
                seq_log_probs = (token_level_log_probs * response_mask).sum(dim=-1)  # (bs,)
                use_ipw = True
                print(f"[GRPO-IPW] Using inverse probability weighted baseline")
            else:
                print("[GRPO-IPW] Warning: use_ipw=True but token_level_log_probs is None. Falling back to standard GRPO.")
        
        for i in range(bsz):
            id2score[index[i]].append(scores[i])
            if use_ipw:
                id2weight[index[i]].append(seq_log_probs[i])
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                if use_ipw:
                    ...
                    # Weight w_i = 1/π_i = exp(-log_π_i)
                    # We want sum(w_i * R_i) / sum(w_i)
                    # To avoid potential overflow with exp(-log_π), we shift exponents.
                    # Let L_i = log_π_i. We want sum(exp(-L_i) * R_i) / sum(exp(-L_i))
                    # Shift by max exponent (-L_min): sum(exp(-L_i - (-L_min)) * R_i) / ...
                    # = sum(exp(L_min - L_i) * R_i) / sum(exp(L_min - L_i))
                    # L_min - L_i is always <= 0, so exp is in (0, 1]. Stable.
                    
                    log_probs_tensor = torch.stack(id2weight[idx]) # (GroupSize,)
                    min_log_prob = torch.min(log_probs_tensor)
                    
                    # weights = exp(min_log_prob - log_prob)
                    # This ensures the largest weight is exp(0) = 1
                    stable_weights = torch.exp(min_log_prob - log_probs_tensor)
                    
                    weighted_sum = (stable_weights * scores_tensor).sum()
                    weight_sum = stable_weights.sum()
                    weighted_mean = weighted_sum / (weight_sum + epsilon)
                    id2mean[idx] = weighted_mean
                    
                    # Weighted standard deviation
                    # Var = sum(w_i * (x_i - mean)^2) / sum(w_i)
                    weighted_var = (stable_weights * (scores_tensor - weighted_mean)**2).sum() / (weight_sum + epsilon)
...
def compute_gppo_advantage(
    ...
    """
    Compute grouped outcome advantages for GPPO.

    Args:
        token_level_rewards: `(torch.Tensor)`
            Sparse reward tensor of shape (bs, response_length).
        values: `(torch.Tensor)`
            Unused placeholder for API compatibility.
        response_mask: `(torch.Tensor)`
            Mask for valid response tokens.
        config: `(Optional[AlgoConfig])`
            Algorithm configuration toggling std normalization.
        token_level_log_probs: `(Optional[torch.Tensor])`
            Placeholder for API compatibility.
        index: `(Optional[np.ndarray])`
            Prompt ids for grouping rollouts.
        epsilon: `(float)`
            Small constant for numerical stability.

    Returns:
        tuple: (token-level advantages, broadcasted outcome returns, scalar advantage targets).
    """
    if index is None:
        raise ValueError("compute_gppo_advantage requires prompt indices for grouping rollouts")

    with torch.no_grad():
        rewards = token_level_rewards.sum(dim=-1)
        gid = as_torch_index(index, device=rewards.device)
        mean_g, std_g, _ = group_mean_std(rewards, gid, eps=epsilon)
        norm_adv = True if config is None else getattr(config, "norm_adv_by_std_in_grpo", True)
        centered = rewards - mean_g[gid]
        if norm_adv:
            centered = centered / (std_g[gid] + epsilon)

        scalar_advantages = centered
