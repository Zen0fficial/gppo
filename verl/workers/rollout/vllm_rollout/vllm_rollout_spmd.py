...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        # Pass logprobs_mode if configured (requires vLLM >= 0.10.2)
        logprobs_mode = config.get("logprobs_mode", None)
        if logprobs_mode is not None:
            if is_version_ge(pkg="vllm", minver="0.10.2"):
                engine_kwargs["logprobs_mode"] = logprobs_mode
                print(f"[vLLM] Using logprobs_mode={logprobs_mode}")
            else:
                print(f"[vLLM] WARNING: logprobs_mode={logprobs_mode} requested but requires vLLM >= 0.10.2. Ignoring.")
