...
from verl.utils.torch_functional import logprobs_from_logits, masked_sum
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def __init__(self, config: ActorConfig, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer = None):
        ...
        self.predict_advantage_head = getattr(self.config, "predict_advantage_head", False)
    ...
    def _forward_micro_batch(
        ...
        if response_mask is not None:
            response_mask = response_mask.to(micro_batch["responses"].device)
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.predict_advantage_head:
                    ...
                    advantage = self._compute_advantage_unpadded(
                ...
                if self.use_fused_kernels:
                    ...
                    if self.predict_advantage_head:
                        last_hidden = self._get_last_hidden_state(output)
                        advantage = self._compute_advantage_padded(
                            last_hidden=last_hidden,
            ...
            self._last_advantage_prediction = advantage
    ...
    def _get_last_hidden_state(self, output):
        ...
        if hidden_states is None:
            raise RuntimeError(
                "predict_advantage_head=True requires the model to return hidden_states. "
    ...
    def _project_advantage_hidden(self, hidden: torch.Tensor) -> torch.Tensor:
        ...
        if proj is None:
            raise RuntimeError("advantage_state_proj is not initialized on the actor module")
        return proj(hidden)
    ...
    def _gather_action_advantage(self, projected_hidden: torch.Tensor, action_token_ids: torch.Tensor) -> torch.Tensor:
        ...
        if head is None:
            raise RuntimeError("advantage_head is not initialized on the actor module")
    ...
    def _compute_advantage_unpadded(
        ...
        adv = self._gather_action_advantage(projected_hidden, action_token_ids)
        if self.use_ulysses_sp:
            adv = gather_outputs_and_unpad(adv, gather_dim=0, unpad_dim=0, padding_size=pad_size)
        adv = pad_input(hidden_states=adv.unsqueeze(-1), indices=indices, batch=batch_size, seqlen=seqlen)
        adv = adv.squeeze(-1)
        adv = adv[:, -response_length - 1 : -1]
        if response_mask is not None:
            adv = adv * response_mask.to(dtype=adv.dtype)
        return adv
    ...
    def _compute_advantage_padded(
        ...
        aligned = self._gather_action_advantage(projected_hidden, action_token_ids)
        if response_mask is not None:
            aligned = aligned * response_mask.to(dtype=aligned.dtype)
        return aligned
        ...
        for _ in range(self.config.ppo_epochs):
            if self.predict_advantage_head:
                ...
                for mini_batch in mini_batches:
                    micro_batches = self._split_micro_batches(mini_batch)
                    for micro_batch in micro_batches:
                        micro_batch = micro_batch.to(get_device_id())
                        model_inputs = {**micro_batch.batch, **micro_batch.non_tensor_batch}
                        ...
                        advantage_predictions = self._last_advantage_prediction
                        if advantage_predictions is None:
                            raise RuntimeError(
                                "predict_advantage_head=True but no advantage predictions were produced."
                            )
                        ...
                        targets = targets.to(advantage_predictions.device)
                        mask = response_mask.to(advantage_predictions.dtype)
                        predicted_sum = masked_sum(advantage_predictions, mask, axis=-1)
                        advantage_loss = F.mse_loss(predicted_sum, targets)
                        loss = advantage_loss * self.config.advantage_loss_coef * loss_scale_factor
                        loss.backward()
                        append_to_dict(metrics, {"actor/advantage_loss": advantage_loss.detach().item()})
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    advantage_predictions = self._last_advantage_prediction if self.predict_advantage_head else None
                    ...
                    if self.predict_advantage_head:
                        ...
                        policy_advantages = advantage_predictions.detach()
                    else:
    ...
    def _split_micro_batches(self, mini_batch: DataProto):
        if self.config.use_dynamic_bsz:
