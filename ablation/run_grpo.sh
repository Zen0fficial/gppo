...
# - IPW (Inverse Probability Weighted) baseline: uses 1/π as weights for baseline
...
EXPERIMENT_NAME="grpo_${TIMESTAMP}"
...
# IPW Baseline Configuration
use_ipw=${USE_IPW:-false}
if [ "$use_ipw" = "true" ]; then
...
# Loss aggregation:
# - Classic GRPO uses per-sequence token mean (each sequence has equal weight).
